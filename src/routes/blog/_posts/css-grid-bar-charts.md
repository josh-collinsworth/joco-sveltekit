...
- Create a grid container (which will serve as our chart), with any number of grid items (children) as the bars in the chart;
- Set `grid-auto-column: 1fr` on the container;
- Color in each bar; and
- Set `grid-column: span x` on each one (where `x` is its value)

Let's start with a simple (albeit contrived) example: a bar chart that shows how many players are on the field at a time in a sport. (Not an especially useful chart, admittedly, but it's some easily understandable data to work with.)

<p class="codepen" data-height="576" data-default-tab="html,result" data-slug-hash="wvPzYrv" data-user="collinsworth" style="height: 576px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;">
...
If you poke around at the code a bit, you'll the chart itself is the grid container. I used an unordered list (`<ul>`) element, and made each bar a list item (`<li>`). This helps to ensure an accessible experience, as the contents of each bar will be read as an individual list item (something like, “_'Baseball: 9,' one of six_”).
...
Each individual bar has a colored background, and uses a `<span>` to separate the name of the bar with its numeric value. That's just so we can use `display: flex` and `justify-content: space-between` to space out the two.

Each individual bar _also_ has `grid-column: span x` set on it as well, where `x` is the bar's value, to cause it to span that many columns in the grid.
...
Using a modern framework like [Svelte](https://svelte.dev), we can create a component to accept that data array as a prop, process it dynamically, and display it as a chart. (If you prever [Vue](https://vuejs.org) instead, we'll look at a different link a little later on.)
...
First, the component will expect a `dataPoints` prop. That's our array of (what else?) data points, each of which will become a bar in the chart. We could add some defaults or type checking here to be safe, if we wanted. That would probably be a good idea in production.
...
That's the heart of the trick; making each bar span X columns, which ensures the chart is always exactly as wide as the widest item(s), saving us from doing any math to figure out how chart items should be sized!
...
CSS grid tries to fit each new item onto the current row if there's space before moving to the next row. That means you could end up with two (or more) of your bars displayed side-by-side, on the same row. You can see this happen in this case if you manually set the grid wide enough, or if one bar is significantly wider than the rest:
...
That's a quick little loop to ensure each bar starts on its own row (as long as you have 10 bars or fewer). That works, but gets less and less viable the more bars you have in your chart. Eventually, that little Sass snippet gets compiled into an unreasonable amount of CSS.
...
The `grid-auto-columns: 1fr` bit is important. Without it, each column may not be sized the same, resulting in possibly skewing the visuals of our chart—and since the whole point of a chart is to show the relationship between items at a precise relative scale, we should definitely be sure the columns are accurately sized.
...
Aside from that, the rest is pretty straightforward, and you can add or adjust depending on your design. `list-style-type` is there just to get rid of the bullets. (You might want to remove the browser's default `padding` on unordered lists, as well.)
...
Here I've shown a horizontal bar chart, but it wouldn't take much changing to make the bar chart vertical instead. The placement of items is just a little more tricky; here's a fork of my pen above:
...
- Also, be sure to set `height: 100%` on each bar.
...
The good news is: as long as you only edit the CSS and don't touch the HTML, you don't the chart will still be accessible!
...
In any case: I hope I've shown you that you don't need to reach for some chart library for simple use cases.
