<script lang="ts">
	...
	function downloadDino() {
		const svgNode = document.getElementById('my-dino') as unknown as SVGSVGElement;
		if (!svgNode) return;
...
<div class="hero-container" bind:this={heroContainer} tabindex="-1">
	...
	<div class="content-grid">
		...
		<div
			...
			style="transform: rotateY({dressupState.isMenuOpen ? '180deg' : '0deg'})"
		>
			<!-- Front: heading text -->
		...
		<!-- Toggle button -->
		<div class="toggle-btn-container" class:btn-hidden={dressupState.isMenuOpen}>
			<button
				type="button"
				onclick={toggleMenu}
				class="toggle-btn"
				tabindex={dressupState.isMenuOpen ? -1 : 0}
				style={dressupState.isMenuOpen ? '' : 'animation: button_bounce 4s ease-in-out infinite;'}
			>
				Make your own dino!
			</button>
		</div>
